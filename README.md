...
**The only complete, portable SDI-12 library available.**
...
No other open-source SDI-12 library is feature complete:
...
- ✅ **Beginner-friendly** — `sdi12_easy.h` convenience macros: sensor in 4
  lines, master in 3 for hobbyists and Arduino users
- ✅ **Pure C11** — no Arduino, no HAL, no OS, no `malloc`
- ✅ **Test coverage** — unit + metamorphic/property-based, all platform-agnostic
- ✅ **Ready for intergration** — works out of the box with PlatformIO Library Manager
...
> Practically all useable SDI-12 libraries only implement the master side, cover a handful of
...
Or clone repo inside the `lib/`. PlatformIO auto-discovers via `library.json`. Then include:
...
Don't want to deal with structs, callbacks tables, and init boilerplate? Include `sdi12_easy.h` and get going in **4 lines**:
...
void my_send(const char *data, size_t length, void *user_data) {
    uart_write(data, length);
}

void my_dir(sdi12_dir_t direction, void *user_data) {
    gpio_set(DIR, direction);
}

sdi12_value_t my_read(uint8_t index, void *user_data) {
    sdi12_value_t value = {0};
    if (index == 0) {
        value.value = read_temp();
        value.decimals = 2;
    }
    return value;
}
...
void on_command(const char *cmd, size_t length) {
    SDI12_SENSOR_PROCESS(my_sensor, cmd, length);      /* 4. Done! */
}
```
...
void read_sensor(char address) {
    SDI12_MASTER_BREAK(recorder);

    sdi12_meas_response_t measurement;
    SDI12_MASTER_MEASURE(recorder, address, &measurement); /* Start measurement */

    if (measurement.wait_seconds > 0) {
        SDI12_MASTER_WAIT(recorder, address, measurement.wait_seconds * 1000);
    }

    sdi12_data_response_t data;
    SDI12_MASTER_GET_DATA(recorder, address, 0, false, &data); /* Read results */
...
(`sdi12_test.h`) — no package manager. Just a C
compiler and `make`.
...
decades. Not one ever released a reusable, complete and open-source implementation of the
protocol they all depend on.

Every embedded engineer needing to use an SDI-12 sensor or create a datalogger has had to
reverse-engineer the spec from scratch, copy snippets from forums, or purchase a
proprietary SDK. For a 1200-baud serial protocol released in 1988.
...
| **Training** | Workshops on embedded best practices |
